...
export default function ParentStudentProgressDetailPage() {
  return (
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([students, sum, top, skl, rec, tr]) => {
        const s = students.find((st: StudentResponse) => st.id === studentId);
        setStudent(s || null);
        setSummary(sum);
      ...
      .catch((e) => {
        ...
        else setError(e.detail ?? "Failed to load progress");
      })
      .finally(() => setLoading(false));
  }, [studentId]);

  if (loading) return <p className="p-8 text-text-secondary">Loading progress...</p>;
  ...
  return (
    <div className="max-w-3xl mx-auto p-8">
      <h1 className="text-2xl font-bold text-text-primary mb-2">
        {student?.display_name ?? "Student"} — Progress
      </h1>
      <p className="text-text-secondary mb-8 text-sm">Year {student?.year_level ?? "—"}</p>

      {/* Summary Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <StatCard label="Exams" value={summary.total_attempts} />
      ...
      <div className="space-x-4">
        <Link href="/parent/progress" className="text-interactive hover:underline text-sm">&larr; All Students</Link>
        <Link href="/parent" className="text-interactive hover:underline text-sm">Dashboard</Link>
      </div>
    </div>
  );
}
